...
for idx, datum in enumerate(hotpot):
    ...
    question_embedding = sentence_embedding(idx, question)
    sentence_count = 1
    for paragraph in datum['corefed_context']:
        ...
        for sentence in content:
            ...
            embedding = sentence_embedding(idx, sentence)
            sentence_similarity = cosine_similarity(embedding, question_embedding)
            datum['numbered_context_flattened'].append([sentence_count, title, sentence, sentence_similarity])
            sentence_count += 1
