...
from wrangle.file_constants import (CLOSEST_FACT_MULTIHOP_FILE,
                                    CLOSEST_FACT_ONETIME_FILE,
                                    CLOSEST_FACT_ORIGINAL_FILE, GLOVE_FILE,
                                    HOTPOT_COREF_FLATTENED_FILE, IDF_FILE,
                                    TF_FILE)
...
for datum in data:
    sp_one_time_ranks = []
    original_pos = []
    ranked_facts = ranker.rank_facts(datum['numbered_context_flattened'],
                                     datum['question'], [])
    ...
    if len(sorted_sp_onetime_ranks) > 1:
        ...
        for idx, fact in enumerate(ranked_facts_second):
            if fact[0] in datum['numbered_supporting_facts'] and fact[
                    0] not in selected_fact_positions:
...
with open(CLOSEST_FACT_ONETIME_FILE, 'w') as fp:
    json.dump(
        {
            "top_sp_ranks": top_sp_onetime_ranks,
            "second_sp_ranks": second_sp_onetime_ranks,
            "third_sp_ranks": third_sp_onetime_ranks
        }, fp)

with open(CLOSEST_FACT_MULTIHOP_FILE, 'w') as fp:
    json.dump(
        {
            "top_sp_ranks": top_sp_multihop_ranks,
            "second_sp_ranks": second_sp_multihop_ranks,
            "third_sp_ranks": third_sp_multihop_ranks
        }, fp)

with open(CLOSEST_FACT_ORIGINAL_FILE, 'w') as fp:
    json.dump(
        {
            "top_original_pos": top_original_pos,
            "second_original_pos": second_original_pos,
            "third_original_pos": third_original_pos
        }, fp)
